...
if len(argv) != 4:
    print("USAGE: %s <transmission_network> <sample_times> <time>"%argv[0]); exit()
END = float(argv[3])
...
if argv[1].endswith('.gz'):
    trans = [l.split('\t') for l in gopen(argv[1]).read().decode().strip().splitlines()]
else:
    trans = [l.split('\t') for l in open(argv[1]).read().strip().splitlines()]
num_trans = {u.split('|')[1]:0 for u in diag if diag[u] <= END}
for u,v,t in trans:
